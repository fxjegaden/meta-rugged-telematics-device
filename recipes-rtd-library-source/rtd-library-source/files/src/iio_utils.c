...
error_free_builtname_generic:
	if( builtname_generic != NULL )
	{
		free(builtname_generic);
		builtname_generic = NULL;
	}
...
error_cleanup_array:
	for (i = count - 1; i >= 0; i--) {
		if( (*ci_array)[i].name != NULL )
		{
			free((*ci_array)[i].name);
			(*ci_array)[i].name = NULL;
		}
		if( (*ci_array)[i].generic_name != NULL )
		{
			free((*ci_array)[i].generic_name);
			(*ci_array)[i].generic_name = NULL;
		}
	}
	if( *ci_array != NULL )
	{
